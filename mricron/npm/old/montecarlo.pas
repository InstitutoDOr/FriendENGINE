...
begin
  //lBinomial := not odd( (Sender as tMenuItem).tag);
          ...
          lPrefs.nCrit := kCrit;
          lPrefs.nPermute := MainForm.ReadPermute;;
        ...
        for lSim := 1 to knSim do begin
	     ...
	     goto 666;
          ...
          if lBinomial then
              ...
              //TurboLDM (lPartImageNames, lMaskHdr, lPrefs, lPartSymptomRA, lFactname,lOutNameSim);
              {$IFDEF anacom}
              AnacomLesionNPMAnalyze (lPartImageNames, lMaskHdr, lnCrit,lSim,lnControlObservations, lPartSymptomRA,lControlSymptomRA, lFactname,lOutNameSim,lPrefs.Ttest,lPrefs.BMtest {lttest,lBM});
              {$ENDIF}
          end;
