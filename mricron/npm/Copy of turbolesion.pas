...
Type
  TLDMPrefs = record
         ...
         nCrit,nPermute,Run{0 except for montecarlo}: integer;
         NameAppend: string;
  end;
...
function TurboLDM (var lImages: TStrings; var lMaskHdr: TMRIcroHdr; lPrefs: TLDMPrefs ; var lSymptomRA: SingleP;var lFactname,lOutName: string): boolean;
label
	123,667;
var
	lOutNameMod: string;
...
begin
	...
	for lPlank := 1 to lnPlanks do begin
        ...
        //lPlankAllocated := false;
        lThreshPermute := 0;
        lnVoxTested := SumThreadData(gnCPUThreads,lPrefs.nPermute,lPermuteMaxT, lPermuteMinT,lPermuteMaxBM, lPermuteMinBM);
        //next report findings
        if lnVoxTested < 1 then begin
	   MainForm.NPMmsg('**Error: no voxels tested: no regions lesioned in at least '+inttostr(lPrefs.nCrit)+' patients**');
           goto 123;
        end;
	...
	MainForm.NPMmsg('Average MEAN effect size = ' +realtostr((lsummean/lmediancount),3));
        ...
        {$ENDIF}
        MainForm.NPMmsg('Only tested voxels with more than '+inttostr(lPrefs.nCrit)+' lesions');
...
//create new header - subsequent images will use Z-scores
        ...
        if (lPrefs.Run < 1) and (Sum2PowerCont(lOutImgSum,lVolVox,lImages.Count)) then begin
...
if lPrefs.ttest then begin //save Ttest
	...
	lOutNameMod := ChangeFilePostfixExt(lOutName,'ttest'+lFactName,'.hdr');
        if lPrefs.Run > 0 then
           MainForm.NPMmsgAppend('threshtt,'+inttostr(lPrefs.Run)+','+inttostr(MainForm.ThreshMap(lThreshNULP,lVolVox,lOutImgT))+','+realtostr(lThreshNULP,3)+','+realtostr(lThreshPermute,3)+','+realtostr(lThreshBonf,3));
...
if lPrefs.BMtest then begin //save Brunner Munzel
        ...
        if lPrefs.Run > 0 then
           MainForm.NPMmsgAppend('threshbm,'+inttostr(lPrefs.Run)+','+inttostr(MainForm.ThreshMap(lThreshNULP,lVolVox,lOutImgBM))+','+realtostr(lThreshNULP,3)+','+realtostr(lThreshPermute,3)+','+realtostr(lThreshBonf,3));
...
123:
        MainForm.FreePermute (lPrefs.nPermute,lPermuteMaxT, lPermuteMinT,lPermuteMaxBM, lPermuteMinBM,  lRanOrderp);
	freemem(lOutImgT);
        freemem(lOutImgAUC);
	freemem(lOutImgBM);
        freemem(lOutImgSum);
	freemem(lObsp);
	   ...
	   freemem(lPlankImg);
        ...
        if lPrefs.NULP then
           lThreshNULP := MainForm.reportBonferroni('Unique overlap',CountOverlap (lImages, lPrefs.nCrit,lnVoxTested));
