...
void SVMProcessing::initializeVars(studyParams &vdb)
{
   ...
   strcpy(cond1, vdb.readedIni.GetValue("FRIEND", "Condition1"));  // obtain names of conditions | JD AUG2016
   strcpy(cond2, vdb.readedIni.GetValue("FRIEND", "Condition2"));
...
// assigns volumes to right condition | 16AUG2016

void SVMProcessing::fillVolumeIndexes(studyParams *&vdbPtr, vector<int>& indices)
{
    for (int t = 0; t <vdbPtr->interval.intervals.size(); t++) // running through all intervals
    {
        if ((strcmp(vdbPtr->interval.intervals[t].condition, cond1)==0) || (strcmp(vdbPtr->interval.intervals[t].condition, cond2)==0)) // in these two intervals
        {
            for (int i=vdbPtr->interval.intervals[t].start+vdbPtr->offset; i <= vdbPtr->interval.intervals[t].end; i++) // add the volume indexes respecting an offset.
            {
                indices.push_back(i);
                //fprintf(stderr, "%d\n", i);
            }
