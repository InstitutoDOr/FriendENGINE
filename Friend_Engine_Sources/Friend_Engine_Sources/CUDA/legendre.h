...
double legendre(double x, int m) {
	/* Legendre polynomials over x = [-1,1]
	 * copied from misc_math.c in afni src
	 */

	if (m < 0)
		return 1.0; /* bad input */
	...
	case 6:
		return (((231.0 * x * x - 315.0) * x * x + 105.0) * x * x - 5.0) / 16.0;
	case 7:
		return (((429.0 * x * x - 693.0) * x * x + 315.0) * x * x - 35.0) * x
				/ 16.0;
	case 8:
		return ((((6435.0 * x * x - 12012.0) * x * x + 6930.0) * x * x - 1260.0)
				* x * x + 35.0) / 128.0;
	...
	case 9:
		return (0.24609375e1
				+ (-0.3609375e2
						+ (0.140765625e3
								+ (-0.20109375e3 + 0.949609375e2 * x * x) * x
										* x) * x * x) * x * x) * x;
