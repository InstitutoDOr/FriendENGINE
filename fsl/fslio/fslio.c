...
#ifdef FSLDLL
static int FslOverrideOutputType = FSL_TYPE_NIFTI;
#else
...
int FslGetEnvOutputType(void)
{
	...
	if (strcmp(otype, "NIFTI") == 0) { if (allocated) free(otype); return FSL_TYPE_NIFTI; }
	if (strcmp(otype, "NIFTI_GZ") == 0) { if (allocated) free(otype); return FSL_TYPE_NIFTI_GZ; }
	if (strcmp(otype, "NIFTI_PAIR") == 0) { if (allocated) free(otype); return FSL_TYPE_NIFTI_PAIR; }
	if (strcmp(otype, "NIFTI_PAIR_GZ") == 0) { if (allocated) free(otype);  return FSL_TYPE_NIFTI_PAIR_GZ; }
...
void FslWriteHeader(FSLIO *fslio)
{
	...
	if (fslio->niftiptr != NULL) {
		...
		strcpy(fslio->niftiptr->descrip, BUILDSTRING);
