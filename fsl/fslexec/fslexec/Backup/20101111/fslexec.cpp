...
extern "C" __declspec(dllimport) int _stdcall NormalizaPorBaseLine(char *Vol4D, char *arqintervalos, char *prefixobasal, char *condicaobasal, char *Saida);
...
extern "C" __declspec(dllimport) int _stdcall AnatomicoResampleado(char *Anatomico, char *sAnatomico, float dx, float dy, float dz, float TR);
...
void CalculaMedias(char *arquivobase, char*arqintervalos)
{
...
void NormalizacaoZ(char *arquivobase, char *mascara, char *saida)
{
	FSLIO *arq;
	char cmd[2000];
...
void NormalizacaoZ2(char *arquivobase, char *mascara, char *saida)
{
	FSLIO *arq;
	char cmd[2000];
	char meanfile[500], stddevfile[500];
...
void preprocessamento(char *arquivo, char *arqintervalo, float FHWM, int highpass, int ativacao, int kernelbox, char *condicaobasal, char **saida)
{
		  ...
		  NormalizaPorBaseLine(SaidaMcFlirt, arqintervalo, prefixobasal, condicaobasal, SaidaMcFlirtAct);
...
*/
	if (argc > 1)
	{
	   ...
	   else if (strcmp(cmd, "SUSAN") == 0) susan(cmdln);
	   ...
	   else if ((strcmp(cmd, "ANATOMICORESAMPLEADO") == 0) && (argc > 7)) AnatomicoResampleado(argv[2], argv[3], (float) atof(argv[4]), (float) atof(argv[5]), (float) atof(argv[6]), (float) atof(argv[7]));
	   ...
	   else if ((strcmp(cmd, "RUNGLM") == 0) && (argc > 3)) 
	   {
		  ...
		  {
		     sprintf(cmd, "fslroi %s %s %d %d", SaidaGauss, treino, 0, fim);
		     fslroi(cmd);
		  }

		  sprintf(Cmd, "fsl_glm -i %s -d %s -c %s -m %s -o %s --out_t=%s", treino, ArqMatriz, ArqContraste, MascaraBin, Saida, Saidat);
		  fsl_glm(Cmd);
...
//		   deslocaarray(classes, total, 1);
		   ...
		   //for (int t=0;t<tamcondicoes;t++)
		   {
			   ...
			   char arquivo[255];
			   sprintf(arquivo, "%s%s.txt", DirSaida, "treino");
