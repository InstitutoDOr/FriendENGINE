        ...
        if (!strncmp(*argv, "-i", 2)) {
            ...
            if (!*++argv)
                ...
                if (wpng_info.gamma <= 0.0)
                    ++error;
                //else if (wpng_info.gamma > 1.01) fprintf(stderr, PROGNAME " warning:  file gammas are usually less than 1.0\n");
            }
...
//                if (argv[1])   /* shouldn't be any more args after filename */
    ...
    if (!inname) {
        if (isatty(0)) {
            //fprintf(stderr, PROGNAME              ":  must give input filename or provide image data via stdin\n");
            ++error;
        } else {
#ifdef DOS_OS2_W32
    ...
    if (!error) {
        ...
        if (pnmline[0] != 'P' || ((pnmchar = pnmline[1]) != '5' &&
            pnmchar != '6' && pnmchar != '8'))
        {
            //fprintf(stderr, PROGNAME ":  input file [%s] is not a binary PGM, PPM or PAM file\n", inname);
            ++error;
        } else {
            ...
            if (wpng_info.width <= 0L || wpng_info.height <= 0L ||
                maxval != 255)
            {
                //fprintf(stderr, PROGNAME ":  only positive width/height, maxval == 255 allowed \n");
                ++error;
            }
...
//                    fflush(stderr);
    ...
    if (wpng_info.interlaced) {
        ...
        if (writepng_encode_image(&wpng_info) != 0) {
           //fprintf(stderr, PROGNAME ":  libpng problem (longjmp) while writing image data\n");
            writepng_cleanup(&wpng_info);
            wpng_cleanup();
            return (2);
        }
        ...
        for (j = wpng_info.height;  j > 0L;  --j) {
            ...
            if (bytes != rowbytes) {
                //fprintf(stderr, PROGNAME  ":  expected %lu bytes, got %lu bytes (row %ld)\n", rowbytes, bytes, wpng_info.height-j);
                ++error;
                break;
            }
